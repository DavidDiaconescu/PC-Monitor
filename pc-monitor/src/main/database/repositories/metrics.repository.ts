...
export class MetricsRepository {
  async save(metric: Metric): Promise<void> {
    ...
    // Calculate disk totals (DiskMetric uses 'size' for total capacity)
    const diskTotal = metric.disk.reduce((sum, d) => sum + d.size, 0)
  ...
  async findLatest(): Promise<Metric | null> {
    ...
    const row = db.prepare('SELECT * FROM metrics ORDER BY timestamp DESC LIMIT 1').get() as
      | MetricRow
      | undefined
